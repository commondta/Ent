                                ...
                                <div class="p-4 code-to-copy">
                                    <div class="table-responsive">
                                        <table class="table table-striped table-sm fs--1 mb-0">
                                            <thead>
                                                <tr>
                                                    <th class="border-top ps-3">Sr #</th>
                                                    <th class="border-top ps-3">Approval</th>
                                                    <th class="border-top ps-3">Stage Name</th>
                                                    <th class="border-top ps-3">No of Approvals</th>
                                            ...
                                            <tbody>
                                                ...
                                                @if($record)
                                                    @foreach($record as $row)
                                                        <tr>
                                                            <td class="align-middle ps-3">{{ $count }}</td>
                                                            <td class="align-middle ps-3">{{ $row->approval }}</td>
                                                            <td class="align-middle ps-3">{{ $row->stage }}</td>
                                                            <td class="align-middle ps-3">{{ $row->no_of_approvals }}</td>
                                                                ...
                                                                <div class="font-sans-serif btn-reveal-trigger position-static">
                                                                    ...
                                                                    <div class="dropdown-menu dropdown-menu-end py-2">
                                                                        ...
                                                                        <a class="dropdown-item"
                                                                            href="{{ route('approval_setup.edit', $row->id) }}">Edit</a>
                                                                        <form id="deleteForm-{{ $row->id }}"
                                                                            action="{{ route('approval_setup.destroy', $row->id) }}"
                                                                            method="Post">
                                                                            @csrf
                                                                            @method('DELETE')
                                                                            <div class="dropdown-divider"></div>
                                                                            <button type="button"
                                                                                onclick="confirmSubmit({{ $row->id }}); return false;"
