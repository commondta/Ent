                                    ...
                                    @if(auth()->user()->undertaking_add == 1)
                                        ...
                                        <div class="col col-md-auto">
                                            <nav class="nav nav-underline justify-content-end doc-tab-nav align-items-center"
                                                ...
                                                <a href="{{ route('undertaking.create') }}"
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-end py-2">
                                                                    @if(auth()->user()->undertaking_edit == 1)
                                                                        ...
                                                                        <a class="dropdown-item"
                                                                            href="{{ route('undertaking.edit', $row->id) }}">Edit</a>

                                                                    @endif
                                                                    @if(auth()->user()->undertaking_print == 1)

                                                                        <a target="_blank" class="dropdown-item"
                                                                            href="{{ route('undertaking.show', $row->id) }}">Layout</a>
                                                                    @endif
                                                                    @if(auth()->user()->undertaking_list == 1)

                                                                        <button type="button" class="dropdown-item"
                                                                            onclick="ViewHistory('<?php        echo $row->id; ?>','Undertaking')">View
                                                                            History</button>
                                                                    @endif
                                                                    @if(auth()->user()->undertaking_edit == 1)

                                                                        <form id="deleteForm-{{ $row->id }}"
                                                                            action="{{ route('undertaking.destroy', $row->id) }}"
                                                                            method="Post">
                                                                            @csrf
                                                                            @method('DELETE')
