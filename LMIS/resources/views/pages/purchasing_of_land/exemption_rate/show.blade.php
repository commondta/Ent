                            ...
                            <div class="card-header p-4 border-bottom border-300 bg-soft">
                                <div class="row g-3 justify-content-between align-items-end">
                                    <div class="col-12 col-md">
                                        <h4 class="text-900 mb-0" data-anchor="data-anchor">Exemption Rates</h4>
                                    </div>
                                    <div class="col col-md-auto">
                                        <nav class="nav nav-underline justify-content-end doc-tab-nav align-items-center"
                                             role="tablist">
                                            @if(auth()->user()->exemption_rate_add == 1)

                                            <a href="{{ route('exemption_rate.create') }}" class="btn btn-sm btn-phoenix-primary preview-btn ms-2" style="width: 190px">
                                                    ...
                                                    <td class="align-middle white-space-nowrap text-end pe-0">
                                                        <div class="font-sans-serif btn-reveal-trigger position-static"><button class="btn btn-sm dropdown-toggle dropdown-caret-none transition-none btn-reveal fs--2" type="button" data-bs-toggle="dropdown" data-boundary="window" aria-haspopup="true" aria-expanded="false" data-bs-reference="parent"><span class="fas fa-ellipsis-h fs--2"></span></button>
                                                            <div class="dropdown-menu dropdown-menu-end py-2">
                                                                @if(auth()->user()->exemption_rate_edit == 1)

                                                                <a class="dropdown-item" href="{{ route('exemption_rate.edit',$row->id) }}">Edit</a>
                                                                @endif
                                                                    ...
                                                                    @endif
                                                                    @if(auth()->user()->exemption_rate_delete == 1)

                                                                    <form id="deleteForm-{{ $row->id }}" action="{{ route('exemption_rate.destroy',$row->id) }}" method="Post">
                                                                    @csrf
                                                                    @method('DELETE')
