                            ...
                            <div class="card-header p-4 border-bottom border-300 bg-soft">
                                <div class="row g-3 justify-content-between align-items-end">
                                    <div class="col-12 col-md">
                                        <h4 class="text-900 mb-0" data-anchor="data-anchor">Land Offer Form (Land Details)
                                        </h4>
                                    </div>
                                                ...
                                                @foreach($record as $row)
                                                    <tr>
                                                        ...
                                                        <td class="align-middle">
                                                            @if($row->loRows && count($row->loRows) > 0)
                                                                @foreach($row->loRows as $lo)
                                                                    <div>{{ $lo->lo_name_as_per_cnic }}</div>
                                                                @endforeach
                                                            @else
                                                                -
                                                            @endif
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-end py-2">
                                                                    @if(auth()->user()->land_form_seller_edit == 1)
                                                                        ...
                                                                        <a class="dropdown-item"
                                                                            href="{{ route('land_form.edit', $row->id) }}">Edit</a>
                                                                    @endif
                                                                    @if(auth()->user()->land_form_seller_list == 1)

                                                                        <button type="button" class="dropdown-item"
                                                                            onclick="ViewHistory('<?php        echo $row->id; ?>','Land Form Seller')">View
                                                                            History</button>
                                                                    @endif
                                                                    @if(auth()->user()->land_form_seller_delete == 1)

                                                                        <form id="deleteForm-{{ $row->id }}"
                                                                            action="{{ route('land_form.destroy', $row->id) }}"
                                                                            method="Post">
                                                                            @csrf
                                                                            @method('DELETE')
