                                    ...
                                    @if(auth()->user()->possession_certificate_add == 1)
                                        ...
                                        <div class="col col-md-auto">
                                            <nav class="nav nav-underline justify-content-end doc-tab-nav align-items-center"
                                                ...
                                                <a href="{{ route('possession_certificate.create') }}"
                                        ...
                                        <table class="table table-striped table-sm fs--1 mb-0">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="border-top ps-3">Land Form No</th>
                                                    <th class="border-top ps-3">Date</th>
                                                    <th class="text-end align-middle pe-0 border-top" scope="col">ACTION
                                                    </th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach($record as $row)
                                                    <tr>
                                                        ...
                                                        <td class="align-middle ps-3">{{ $row->base_code_no }}</td>
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-end py-2">
                                                                    @if(auth()->user()->possession_certificate_edit == 1)
                                                                        ...
                                                                        <a class="dropdown-item"
                                                                            href="{{ route('possession_certificate.edit', $row->id) }}">Edit</a>
                                                                    @endif
                                                                    @if(auth()->user()->possession_certificate_list == 1)

                                                                        <button type="button" class="dropdown-item"
                                                                            onclick="ViewHistory('<?php        echo $row->id; ?>','Possession Certificate')">View
                                                                            History</button>
                                                                    @endif
                                                                    @if(auth()->user()->possession_certificate_print == 1)

                                                                        <a target="_blank" class="dropdown-item"
                                                                            href="{{ route('possession_certificate.show', $row->id) }}">Layout</a>
                                                                    @endif
                                                                    @if(auth()->user()->possession_certificate_delete == 1)

                                                                        <form id="deleteForm-{{ $row->id }}"
                                                                            action="{{ route('possession_certificate.destroy', $row->id) }}"
                                                                            method="Post">
                                                                            @csrf
                                                                            @method('DELETE')
