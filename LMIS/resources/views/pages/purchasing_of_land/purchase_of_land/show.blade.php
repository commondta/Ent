                            ...
                            <div class="row g-3 justify-content-between align-items-end">
                                ...
                                <div class="col col-md-auto">
                                    <nav class="nav nav-underline justify-content-end doc-tab-nav align-items-center"
                                        ...
                                        <a href="{{ route('purchase_of_land.create') }}" class="btn btn-sm btn-phoenix-primary preview-btn ms-2" style="width: 190px">
                            ...
                            <div class="p-4 code-to-copy">
                                <div class="table-responsive">
                                    <table class="table table-striped table-sm fs--1 mb-0">
                                        <thead>
                                            <tr>
                                                <th class="sort border-top ps-3">File NO</th>
                                                <th class="sort border-top ps-3">Land Form NO</th>
                                                <th class="sort border-top ps-3">Doc Date</th>
                                                <!-- <th class="sort border-top" data-sort="email">LO Name</th> -->
                                                <!-- <th class="sort border-top" data-sort="age">LO CNIC</th> -->
                                                <th class="sort border-top">District Amount</th>
                                                <th class="sort border-top">{{ config('app.org_label') }} Amount</th>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td class="align-middle ps-3">{{ $row->File_No }}</td>
                                                <td class="align-middle ps-3">{{ $row->land_form_no }}</td>
                                                <td class="align-middle ps-3">{{ $row->doc_date }}</td>
                                                <!-- <td class="align-middle">{{ $row->lo_name }}</td> -->
                                              
                                                <!-- <td class="align-middle">{{ $row->lo_cnic }}</td> -->
                                                <td class="align-middle">{{ $row->district_amount }}</td>
                                                <td class="align-middle">{{ $row->society_amount }}</td>
                                                <td class="align-middle white-space-nowrap text-end pe-0">
                                                    <div class="font-sans-serif btn-reveal-trigger position-static"><button class="btn btn-sm dropdown-toggle dropdown-caret-none transition-none btn-reveal fs--2" type="button" data-bs-toggle="dropdown" data-boundary="window" aria-haspopup="true" aria-expanded="false" data-bs-reference="parent"><span class="fas fa-ellipsis-h fs--2"></span></button>
                                                        <div class="dropdown-menu dropdown-menu-end py-2">
                                                            @if(auth()->user()->purchase_of_land_edit == 1)

                                                            <a class="dropdown-item" href="{{ route('purchase_of_land.edit',$row->id) }}">Edit</a>
                                                            @endif
                                                            ...
                                                            <a target="_blank" class="dropdown-item" href="{{ route('purchase_of_land.show',$row->id) }}">Layout</a>
                                                            @endif
                                                            @if(auth()->user()->purchase_of_land_delete == 1)

                                                            <form id="deleteForm-{{ $row->id }}" action="{{ route('purchase_of_land.destroy',$row->id) }}" method="Post">
                                                                @csrf
                                                                @method('DELETE')
                                                                <div class="dropdown-divider"></div>
                                                                <button onclick="confirmSubmit(); return false;" class="dropdown-item text-danger">Remove</button>
                                                                {{--<a type="submit" class="dropdown-item text-danger" href="#!"></a>--}}
