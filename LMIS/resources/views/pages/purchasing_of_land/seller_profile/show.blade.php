                            ...
                            <div class="card-header p-4 border-bottom border-300 bg-soft">
                                <div class="row g-3 justify-content-between align-items-end">
                                    <div class="col-12 col-md">
                                        <h4 class="text-900 mb-0" data-anchor="data-anchor">Land Owner Profile Master Data
                                        </h4>
                                    </div>
                                    @if(auth()->user()->seller_profile_add == 1)
                                        ...
                                        <div class="col col-md-auto">
                                            <nav class="nav nav-underline justify-content-end doc-tab-nav align-items-center"
                                                ...
                                                <a href="{{ route('seller_profile.create') }}"
                                            ...
                                            <tbody>
                                                @foreach($record as $row)
                                                    <tr>
                                                        ...
                                                        <td class="align-middle">{{ $row->lo_name_as_per_cnic }}</td>
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-end py-2">
                                                                    @if(auth()->user()->seller_profile_edit == 1)
                                                                        ...
                                                                        <a class="dropdown-item"
                                                                            href="{{ route('seller_profile.edit', $row->id) }}">Edit</a>
                                                                    @endif
                                                                    @if(auth()->user()->seller_profile_list == 1)

                                                                        <button type="button" class="dropdown-item"
                                                                            onclick="ViewHistory('<?php        echo $row->id; ?>','Seller Profile')">View
                                                                            History</button>
                                                                    @endif
                                                                    @if(auth()->user()->seller_profile_delete == 1)

                                                                        <form id="deleteForm-{{ $row->id }}"
                                                                            action="{{ route('seller_profile.destroy', $row->id) }}"
                                                                            method="Post">
                                                                            @csrf
                                                                            @method('DELETE')
