                                                ...
                                                @if(isset($record))
                                                    @foreach($record as $row)
                                                        <tr>
                                                            ...
                                                            <td class="align-middle">{{ $row->agreement_date }}</td>
                                                                    ...
                                                                    <div class="dropdown-menu dropdown-menu-end py-2">
                                                                        @if(auth()->user()->agreement_edit == 1)
                                                                            ...
                                                                            <a class="dropdown-item"
                                                                                href="{{ route('agreement.edit', $row->id) }}">Edit</a>

                                                                        @endif
                                                                        @if(auth()->user()->agreement_print == 1)

                                                                            <a target="_blank" class="dropdown-item"
                                                                                href="{{ route('agreement.show', $row->id) }}">Layout</a>
                                                                        @endif
                                                                        @if(auth()->user()->agreement_list == 1)

                                                                            <button type="button" class="dropdown-item"
                                                                                onclick="ViewHistory('<?php            echo $row->id; ?>','Agreement')">View
                                                                                History</button>
                                                                        @endif
                                                                        @if(auth()->user()->agreement_delete == 1)

                                                                            <form id="deleteForm-{{ $row->id }}"
                                                                                action="{{ route('agreement.destroy', $row->id) }}"
                                                                                method="Post">
                                                                                @csrf
                                                                                @method('DELETE')
