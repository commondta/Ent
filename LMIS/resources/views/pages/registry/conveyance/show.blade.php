                            ...
                            <div class="card-header p-4 border-bottom border-300 bg-soft">
                                <div class="row g-3 justify-content-between align-items-end">
                                    <div class="col-12 col-md">
                                        <h4 class="text-900 mb-0" data-anchor="data-anchor">Conveyance</h4>
                                    </div>
                                                ...
                                                @if(isset($record))
                                                    @foreach($record as $row)
                                                        <tr>
                                                            ...
                                                            <td class="align-middle">{{ $row->date_of_creation }}</td>
                                                                    ...
                                                                    <div class="dropdown-menu dropdown-menu-end py-2">
                                                                        @if(auth()->user()->conveyance_deed_edit == 1)
                                                                            ...
                                                                            <a class="dropdown-item"
                                                                                href="{{ route('conveyance.edit', $row->id) }}">Edit</a>

                                                                        @endif
                                                                        @if(auth()->user()->conveyance_deed_print == 1)

                                                                            <a target="_blank" class="dropdown-item"
                                                                                href="{{ route('conveyance.show', $row->id) }}">Layout</a>

                                                                        @endif
                                                                        @if(auth()->user()->conveyance_deed_list == 1)

                                                                            <button type="button" class="dropdown-item"
                                                                                onclick="ViewHistory('<?php            echo $row->id; ?>','Conveyance Deed')">View
                                                                                History</button>
                                                                        @endif
                                                                        @if(auth()->user()->conveyance_deed_delete == 1)

                                                                            <form id="deleteForm-{{ $row->id }}"
                                                                                action="{{ route('conveyance.destroy', $row->id) }}"
                                                                                method="Post">
                                                                                @csrf
                                                                                @method('DELETE')
