                                    ...
                                    @if(auth()->user()->registry_document_add == 1)
                                        ...
                                        <div class="col col-md-auto">
                                            <nav class="nav nav-underline justify-content-end doc-tab-nav align-items-center"
                                                ...
                                                <a href="{{ route('registry_document.create') }}"
                                        ...
                                        <table class="table table-striped table-sm fs--1 mb-0">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="border-top">Registry No</th>
                                                    <th class="border-top">Mutation No</th>
                                                ...
                                                @if(isset($record))
                                                    @foreach($record as $row)
                                                        <tr>
                                                            ...
                                                            <td class="align-middle">{{ $row->registry_no }}</td>
                                                            <td class="align-middle">{{ $row->mutation_no }}</td>
                                                                    ...
                                                                    <div class="dropdown-menu dropdown-menu-end py-2">
                                                                        @if(auth()->user()->registry_document_edit == 1)
                                                                            ...
                                                                            <a class="dropdown-item"
                                                                                href="{{ route('registry_document.edit', $row->id) }}">Edit</a>

                                                                        @endif
                                                                        @if(auth()->user()->registry_document_list == 1)

                                                                            <button type="button" class="dropdown-item"
                                                                                onclick="ViewHistory('<?php            echo $row->id; ?>','Registry Document')">View
                                                                                History</button>
                                                                        @endif
                                                                        @if(auth()->user()->registry_document_delete == 1)

                                                                            <form id="deleteForm-{{ $row->id }}"
                                                                                action="{{ route('registry_document.destroy', $row->id) }}"
                                                                                method="Post">
                                                                                @csrf
                                                                                @method('DELETE')
