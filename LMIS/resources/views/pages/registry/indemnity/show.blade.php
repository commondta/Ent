                                    ...
                                    @if(auth()->user()->indemnity_bond_add == 1)
                                        ...
                                        <div class="col col-md-auto">
                                            <nav class="nav nav-underline justify-content-end doc-tab-nav align-items-center"
                                                ...
                                                <a href="{{ route('indemnity_bond.create') }}"
                                        ...
                                        <table class="table table-striped table-sm fs--1 mb-0">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="border-top ps-3">Date</th>
                                                    <th class="border-top">Base Doc No</th>
                                                    <th class="border-top">Date of Execution</th>
                                                    <th class="text-end align-middle pe-0 border-top">
                                                        ACTION
                                                    </th>
                                                </tr>
                                            </thead>
                                                ...
                                                @if(isset($record))
                                                    @foreach($record as $row)
                                                        <tr>
                                                            ...
                                                            <td class="align-middle">{{ $row->base_doc_no }}</td>
                                                            <td class="align-middle">{{ $row->date_of_execution }}</td>
                                                                    ...
                                                                    <div class="dropdown-menu dropdown-menu-end py-2">
                                                                        @if(auth()->user()->indemnity_bond_edit == 1)
                                                                            ...
                                                                            <a class="dropdown-item"
                                                                                href="{{ route('indemnity_bond.edit', $row->id) }}">Edit</a>

                                                                        @endif @if(auth()->user()->indemnity_bond_print == 1)

                                                                            <a target="_blank" class="dropdown-item"
                                                                                href="{{ route('indemnity_bond.show', $row->id) }}">Layout</a>

                                                                        @endif
                                                                        @if(auth()->user()->indemnity_bond_list == 1)

                                                                            <button type="button" class="dropdown-item"
                                                                                onclick="ViewHistory('<?php            echo $row->id; ?>','Indemnity Bond')">View
                                                                                History</button>
                                                                        @endif
                                                                        @if(auth()->user()->indemnity_bond_delete == 1)

                                                                            <form id="deleteForm-{{ $row->id }}"
                                                                                action="{{ route('indemnity_bond.destroy', $row->id) }}"
                                                                                method="Post">
                                                                                @csrf
                                                                                @method('DELETE')
