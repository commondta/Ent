using System;
using Microsoft.EntityFrameworkCore.Migrations;

#nullable disable

namespace B_DB_Context.Migrations
{
    public partial class NewDemarcationRequest_added : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.CreateTable(
                name: "NewDemarcationRequest",
                columns: table => new
                {
                    Id = table.Column<int>(type: "int", nullable: false)
                        .Annotation("SqlServer:Identity", "1, 1"),
                    Status = table.Column<string>(type: "nvarchar(max)", nullable: true),
                    GlobalChargeSetupId = table.Column<int>(type: "int", nullable: true),
                    StockCreationId = table.Column<int>(type: "int", nullable: true),
                    IsActive = table.Column<bool>(type: "bit", nullable: false),
                    IsDeleted = table.Column<bool>(type: "bit", nullable: false),
                    CreatedOn = table.Column<DateTime>(type: "datetime2", nullable: false),
                    CreatedBy = table.Column<int>(type: "int", nullable: true),
                    LastModified = table.Column<DateTime>(type: "datetime2", nullable: false),
                    ModifiedBy = table.Column<int>(type: "int", nullable: true)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_NewDemarcationRequest", x => x.Id);
                    table.ForeignKey(
                        name: "FK_NewDemarcationRequest_GlobalChargeSetup_GlobalChargeSetupId",
                        column: x => x.GlobalChargeSetupId,
                        principalTable: "GlobalChargeSetup",
                        principalColumn: "Id");
                    table.ForeignKey(
                        name: "FK_NewDemarcationRequest_StockCreations_StockCreationId",
                        column: x => x.StockCreationId,
                        principalTable: "StockCreations",
                        principalColumn: "ID");
                });

            migrationBuilder.CreateIndex(
                name: "IX_NewDemarcationRequest_GlobalChargeSetupId",
                table: "NewDemarcationRequest",
                column: "GlobalChargeSetupId");

            migrationBuilder.CreateIndex(
                name: "IX_NewDemarcationRequest_StockCreationId",
                table: "NewDemarcationRequest",
                column: "StockCreationId");
        }

        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropTable(
                name: "NewDemarcationRequest");
        }
    }
}
