...
function sampleAcquired(s) {
    if (currentFormat == Fingerprint.SampleFormat.PngImage) {
        ...
        //var finger = "data:image/png;base64," + Fingerprint.b64UrlTo64(samples[0]);
        ...
        var FingerId = $('input[name="FingerName"]:checked').val();
        var memberProfileId = $("#memberProfileId").val();
        var base64 = Fingerprint.b64UrlTo64(samples[0]);
        var ObjFingers = new Object();
        ObjFingers.FingerId = FingerId;
        ObjFingers.MemberProfileId = memberProfileId;
        ObjFingers.EncodedBase64 = base64;
        ObjFingers.ModifiedBy = $("#User_Id").val();
        ObjFingers.LastModifiedUserName = $("#User_Name").val();
        let fingerIdExists = Fingers.some(item => item.FingerId === ObjFingers.FingerId);

        if (fingerIdExists == false) {
            Fingers.push(ObjFingers);
            onSaveFinger();
        }
        else {
            alert("Finger Print Alredy Exsist");
        }
...
function populatePopUpModal() {
    ...
    if (myVal != "") {
        ...
        modelWindowElement.innerHTML = "Please select a reader and member";
    }
}
...
function disableEnable() {
    ...
    if (myVal != "" && member != "") {
        ...
        showMessage("Please select a reader and member");
        onStop();
    }
}
...
function GetAllMembersList() {
    ...
    $('#memberlist').DataTable({
        ajax: {
            url: "/api/SP/GetAllMemberProfiles",
            type: "POST",
            headers: {
        ...
        columns: [
            ...
            { data: "cnic", name: "CNIC" },
            { data: "relationshipWith", name: "RelationshipWith" },
            { data: "mobile", name: "Mobile" },
            { data: "dateTime", name: "dateTime" },
            {
                data: null,
                render: function (data, type, row) {
                    return '<a class="btn btn-primary" onclick="SelectMember(' + data.id + ')">Select</a>';
                }
            }
        ]
    });
}
function SaveFingers() {
    console.log(Fingers);

    // Check if Fingers is defined and not empty
    if (!Fingers || Fingers.length === 0) {
        console.error('Fingers is null or empty. Aborting the request.');
        return;
    }

    const jsonData = JSON.stringify(Fingers);

    $.ajax({
        type: "POST",
        url: '/FingerPrint/StoreFingerprintData',
        ...
        success: function (data) {
            if (data == true) {
                alert('Finger Print Saved.');
            }
            else {
                alert("Member Finger Print Alrady exist")
            }
        },
        error: function () {
            alert('Error');
