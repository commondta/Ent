...
function sampleAcquired(s) {
    if (currentFormat == Fingerprint.SampleFormat.PngImage) {
        ...
        var verificationType = $("#VType").val();
        var base64 = Fingerprint.b64UrlTo64(samples[0]);
        var modifiedBy = $("#User_Id").val();
        var lastModifiedUserName = $("#User_Name").val();
        $.ajax({
            type: "POST",
            url: '/FingerPrint/Compare',
            data: { fingerData: base64, FingerId: FingerId, MemberProfileId: memberProfileId, VerificationType: verificationType, LastModifiedUserName: lastModifiedUserName, ModifiedBy: modifiedBy },
        ...
        $.ajax({
            type: "POST",
            url: '/Home/Compare',
            data: { fingerData: Fingerprint.b64UrlTo64(samples[0]) },
            success: function (data) {
                if (data === true)
                    alert('You are matched.');
                else {
